...
async function total(list) {  
    ...
    for (const item of list) {
        const event = await AdEvent.find(JSON.parse('{"event_number" : '+'"'+item.toString()+'"'+'}'))
        counts = counts + ('"event_number '+item.toString()+'"'+" : "+event.length.toString()+ ", ")
    }
...
exports.getVersion = async (req, res, next) => {
    try{
        const version = await Ad.find(req.body);
        return res.status(200).json({
            success: true, 
...
exports.getTotals = async (req, res, next) =>{ 
    ...
    try{
        const num = await AdEvent.distinct("event_number", function(err, events) { // gets distinct event numbers 
...
exports.funs = async(req, res, next) =>{
    ...
    try {
        ...
        var result = []
        var temp = []
        for(const sess of sessions) { // could be handled better 
            const event = await AdEvent.find({"event_name":{$in:req.body.order},"event_number":{$gt:0, $lt:req.body.order.length + 1 },"session": sess.id})
            if(event.length != 0) {
...
function countings(funnel, events, index) {
    ...
    for (var i = 0, l = events.length; i<l; i++) {
        ...
        if(JSON.stringify(funnel[index]) === JSON.stringify(events[i].events[n].event_name) && events[i].events[n].event_number === (index+1)) {
